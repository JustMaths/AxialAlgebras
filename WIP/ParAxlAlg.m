...
Some basic operations of partial axial algebra elements
...
A useful function for giving the dimensions of all the eigenspaces
...
intrinsic Axes(A::ParAxlAlg) -> SetIndx, Map
  ...
  for i in [1..#A`axes] do
    pairs := {@ <A`axes[i]`id*g, A`axes[i]`inv^g> : g in Transversal(Group(A), A`axes[i]`stab) @};
...
intrinsic mClosed(A::ParAxlAlg) -> RngIntElt
  ...
  G := Group(A);
  W := A`W;
  axes := &cat[ [ (A`axes[i]`id*g)`elt : g in Transversal(G, A`axes[i]`stab) ] : i in [1..#A`axes] ];
